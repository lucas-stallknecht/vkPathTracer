...
#include "core/utils/obj_loader.h"
...
namespace path_tracing
{
    void TraceMeshBuilder::setGeometry(const std::string& objPath)
    {
        ...
        core_utils::loadTraceableGeometryFromObj(objPath, vertices_, triangles_);
    }

    void TraceMeshBuilder::setMaterial(const MaterialProperties& material)
    {
        material_ = material;
    }

    Mesh TraceMeshBuilder::build(uint32_t depth)
    {
        maxDepth_ = depth;
        nodes_.clear();
        buildBVH();
        return Mesh{vertices_, triangles_, material_, nodes_};
    }
    ...
    float TraceMeshBuilder::giveSplitPosAlongAxis(int axis, const BVHNode& node)
    {
        float sum = 0;
